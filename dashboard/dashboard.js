...
// Charger les lectures enregistrées
function loadUserReadings() {
    const user = JSON.parse(localStorage.getItem('currentUser') || '{"email":"guest"}');
    const data = JSON.parse(localStorage.getItem(`userData_${user.email}`) || '{}');
    if (data.hba1c) document.getElementById('hba1c').textContent = data.hba1c;
    if (data.bp) document.getElementById('bp').textContent = data.bp;
    if (data.cholesterol) document.getElementById('cholesterol').textContent = data.cholesterol;
    if (data.fastingSugar) document.getElementById('fastingSugar').textContent = data.fastingSugar;
}
